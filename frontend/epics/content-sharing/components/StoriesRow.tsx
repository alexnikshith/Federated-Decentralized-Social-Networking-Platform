...
export const StoriesRow: React.FC = () => {
    ...
    const { stories, loading, fetchStories, markStoryViewed, getViewedStoryIDs } = useStoryStore();
            ...
            >
                <div className="flex overflow-x-auto scrollbar-hide py-2 relative" style={{ scrollBehavior: 'smooth', minHeight: '180px' }}>
                    <div className="relative" style={{ width: contentWidth, minHeight: '180px' }}>
                        ...
                        {/* Other User Story Nodes or Skeletons */}
                        {loading && groupedStories.length === 0 ? (
                            [1, 2, 3, 4, 5].map((idx) => {
                                const xPos = 100 + (idx) * itemWidth;
                                const pos = getPoint(xPos);
                                return (
                                    <div
                                        key={`skeleton-${idx}`}
                                        className="absolute flex flex-col items-center z-10 -translate-x-1/2 -translate-y-1/2 opacity-70"
